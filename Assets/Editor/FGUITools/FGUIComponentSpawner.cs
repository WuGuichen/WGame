...
{
    public static class FGUIComponentSpawner
    {
        ...
        public static void SpawnComponent(ComponentInfo componentInfo)
        {
            ...
            FGUICodeSpawner.ExportedComponentInfos.Add(componentInfo.PackageId, componentInfo.Id, componentInfo);
            ...
            sb.AppendLine();
            sb.AppendLine("{");
            sb.AppendFormat("\tpublic partial class {0}{1}: {2}", FGUICodeSpawner.ClassNamePrefix, componentInfo.NameWithoutExtension, componentInfo.ComponentClassName);
            sb.AppendLine();
            sb.AppendLine("\t{");
            ...
            sb.AppendFormat("\t\tpublic static {0}{1} CreateInstance()", FGUICodeSpawner.ClassNamePrefix, componentInfo.NameWithoutExtension);
            sb.AppendLine();
            sb.AppendLine("\t\t{");
            sb.AppendFormat("\t\t\treturn ({0}{1})UIPackage.CreateObject(\"{2}\", \"{1}\");", 
                FGUICodeSpawner.ClassNamePrefix, 
                componentInfo.NameWithoutExtension,
                FGUICodeSpawner.PackageInfos[componentInfo.PackageId].Name);
            sb.AppendLine();
            sb.AppendLine("\t\t}");
            sb.AppendLine();
            ...
            string dir = "{0}/{1}".Fmt(FGUICodeSpawner.FUIAutoGenDir, FGUICodeSpawner.PackageInfos[componentInfo.PackageId].Name);
            if (!Directory.Exists(dir))
            {
                Directory.CreateDirectory(dir);
            }
            
            string filePath = "{0}/{1}{2}.cs".Fmt(dir, FGUICodeSpawner.ClassNamePrefix, componentInfo.NameWithoutExtension);
            using FileStream fs = new FileStream(filePath, FileMode.Create);
            using StreamWriter sw = new StreamWriter(fs);
            sw.Write(sb.ToString());
            SpawnItemCode(componentInfo);
        }

        private static void SpawnItemCode(ComponentInfo componentInfo)
        {
        ...
        {
            if (FGUICodeSpawner.IgnoreDefaultVariableName && variableInfo.IsDefaultName)
            {
