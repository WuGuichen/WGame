...
namespace WGame.UIEditor
{
    ...
    public class PackageInfo
    {
        ...
        public Dictionary<string, PackageComponentInfo> PackageComponentInfos { get; set; } =
            new Dictionary<string, PackageComponentInfo>();
    }
    ...
    public class ComponentInfo
    {
        ...
        // // 是否需要导出item类
        // public bool NeedExportComponentItemClass { get; private set; }
        ...
        public void CheckCanExport(HashSet<string> ExtralExportURLs, bool IgnoreDefaultVariableName)
        {
            ...
            if (needExportClass)
            {
                NameSpace = "{0}.{1}".Fmt(FGUICodeSpawner.NameSpace, FGUICodeSpawner.PackageInfos[PackageId].Name);
                ComponentTypeName = "{0}{1}".Fmt(FGUICodeSpawner.ClassNamePrefix, NameWithoutExtension);
            }
            else
            {
        ...
        public void SetVariableInfoTypeName()
        {
            for (int index = 0; index < this.VariableInfos.Count; index++)
            {
                ...
                if (FGUICodeSpawner.ComponentInfos.TryGetValue(key, out ComponentInfo componentInfo))
                {
        ...
        private static string GetTypeNameByDisplayXML(string parentPackageId, XML displayXML)
        {
            ...
            if (displayXML.name == "component")
            {
                ...
                ComponentInfo displayComponentInfo = FGUICodeSpawner.ComponentInfos[key];
